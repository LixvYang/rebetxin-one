...
func (l *ListTopicCollectLogic) ListTopicCollect() ([]types.Topiccollect, error) {
	...
	for i := 0; i < len(respData); i++ {
		...
		respData[i].Topic.CreatedAt = l.svcCtx.TimeToString(topic.CreatedAt)
		respData[i].Topic.DeletedAt = l.svcCtx.TimeToString(topic.DeletedAt.Time)
		respData[i].Topic.EndTime = l.svcCtx.TimeToString(topic.EndTime.Time)
